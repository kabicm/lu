...
#SBATCH --nodes=%d \n\
#SBATCH --output=%s/mkl-weak-%s-p%d-%s.txt \n\
...
def readConfig(section):
    ...
    if len(N) ==0 or len(v) == 0 or len(grids) == 0 or len(N) != len(grids):
    ...
    for idx, grid in enumerate(grids):
        ...
        with open(filename, 'w') as f:
            ...
            for rectangles in grids[grid]:
                for n in N[idx]:
                    for v in V:
                        pre = 'export SCOREP_EXPERIMENT_DIRECTORY=benchmarks/scorep/weak_%s_%d_%d_%d_%s \n' %(algorithm, n, grid, v, rectangles) 
